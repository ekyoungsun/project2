...
<?
    ...
    /*  단일 파일 업로드 
		$upfile_name	 = $_FILES["upfile"]["name"]; 파일 이름
		$upfile_tmp_name = $_FILES["upfile"]["tmp_name"]; 임시저장 파일
		$upfile_type     = $_FILES["upfile"]["type"]; 파일 종류
		$upfile_size     = $_FILES["upfile"]["size"]; 파일 크기(용량)
		$upfile_error    = $_FILES["upfile"]["error"]; 파일 처리가 제대로 되었는지
    */
	...
	for($i = 0; $i < $count; $i++){
        $upfile_name[$i] = $files["name"][$i];  // 파일 이름(a1.jpg)
		$upfile_tmp_name[$i] = $files["tmp_name"][$i];  // 임시저장 파일
		$upfile_type[$i] = $files["type"][$i];  // 파일 종류
		$upfile_size[$i] = $files["size"][$i];  // 파일 크기(용량)
		$upfile_error[$i] = $files["error"][$i];  // 파일 처리가 제대로 되었는지
      
		$file = explode(".", $upfile_name[$i]);  // explode(".", ~); .을 기준으로 찾음  ex) a1.jpg
		$file_name = $file[0];  // a1
		$file_ext = $file[1];  // jpg
		...
		if(!$upfile_error[$i]){  // 에러가 발생되지 않으면
			...
			if($upfile_size[$i] > 500000){  // 파일 크기(용량)제한 ex) 500000 = 500KB
				echo("<script>
                        alert('업로드 파일 크기가 지정된 용량(500KB)을 초과했습니다.');
                        history.go(-1)
                      </script>");
				exit;
			}
			if(($upfile_type[$i] != "image/gif") && ($upfile_type[$i] != "image/jpeg") && ($upfile_type[$i] != "image/pjpeg")){  // ie는 pjpeg
				echo("<script>
						alert('JPG와 GIF이미지 파일만 업로드 가능합니다.');
						history.go(-1)
					  </script>");
				exit;
			}
			if(!move_uploaded_file($upfile_tmp_name[$i], $uploaded_file[$i])){
            // move_uploaded_file(임시저장 파일명, 실제저장할 서버경로 파일명) => 파일 업로드
            // 파일을 업로드하고 업로그 처리가 안 되었을 때    
				echo("<script>
    ...
    echo "<script>
            location.href = '../../../subForm.php?idx=ceoNews&pages=notice&table$table&page=$page';
          </script>";
?>